    ...
    <el-form ref="searchForm">
      ...
      <el-form-item>
        ...
        <el-input type="text" placeholder="请输入视频标题进行搜索.." clearable
          ...
          -->
          <template #append>
            <el-button :icon="Search" @click="searchVideo(videoTitle,1,videoPageInfo['page-size'])"/>
          </template>
...
// method: 异步分页查询视频列表
let searchVideo = async (title, page, size) => {
  try {
    if ('' === title.trim()) {
      title = 'JB';
    }
...
let changePage = page => {
  ...
  searchVideo(videoTitle.value, page, videoPageInfo['page-size']);
};

// method: 每页显示多少条被改变时触发：
let changeSize = size => {
  videoPageInfo['page-size'] = size;
  searchVideo(videoTitle.value, videoPageInfo['page-num'], size);
};

// mounted: 页面加载完毕后，立刻调用 `searchVideo()` 方法
onMounted(() => searchVideo('JB', 1, 6));
